...
class MainRecommender:
    ...
    def __init__(
        ...
        self.top_purchases = (
            data.groupby(["user_id", "item_id"])["quantity"].count().reset_index()
        )
        self.top_purchases.sort_values("quantity", ascending=False, inplace=True)
        self.top_purchases = self.top_purchases[self.top_purchases["item_id"] != 999999]

        # Top purchases of all dataset
        self.overall_top_purchases = (
    ...
    def get_similar_items_recommendation(self, user, N=5):
        ...
        res = (
            top_users_purchases["item_id"]
            .apply(lambda x: self._get_similar_item(x))
            .tolist()
        )
        res = self._extend_with_top_popular(res, N=N)
    ...
    def get_similar_users_recommendation(self, user, N=5):
        ...
        similar_user = self.model.similar_users(self.userid_to_id[user], N=2)[
            0
        ].tolist()[1]
