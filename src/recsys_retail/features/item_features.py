...
def item_features_frequency_encoder(item_features: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    frequency_encoder = make_pipeline(
        ce.count.CountEncoder(cols=cols, normalize=True), MinMaxScaler()
    )
...
def fit_transform_item_features(
    item_features: pd.DataFrame, vectorize_desc=False
) -> pd.DataFrame:
    """
